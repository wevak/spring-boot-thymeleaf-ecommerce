...
//            )
            .authorizeHttpRequests(request->request.requestMatchers("/admin/**")
    				.hasAuthority("ADMIN").requestMatchers("/cart", "/user/cart/**").hasAuthority("USER")
    				.requestMatchers("/","/home","/shop/**","/register", "/register/user","/css/**","/images/**","/productImages/**", "/user/category/**", "/user/products").permitAll()
    				.anyRequest().authenticated())
